...
def drawEvents(data, drop_radius, pdf,n):
    ...
    for event_id, event in data_grouped:
        ...
        sc1 = axs1.scatter(event['x'], event['y'], marker='o', s=abs(event['rawDriftTime'])/25  , c=event['currentTrackPID'], cmap='rainbow', alpha=0.3, label=event['currentTrackPID'])
            ...
            for particleId, particle in track_particle:
                ...
                if trackId >= 0:
                    ...
                    if charge < 0:
                        ...
                        #带正电粒子径迹圆原始数据
                        circle = Circle([cx, cy], r, color='skyblue', fill=False, alpha=1)
                        axs1.add_patch(circle)
        ...
        axs2.set_ylim((-82, 82))
        axs2.set_aspect(1)
        sc2 = axs2.scatter(event['x'], event['y'], marker='o', s=abs(event['rawDriftTime'])/25 , c=event['trackIndex'], cmap='rainbow', alpha=0.6, label=event['trackIndex'])
        ...
        sc3 = axs3.scatter(event['x'], event['y'], marker='o', s=abs(event['rawDriftTime']) / 25,
...
def main():
    ...
    output = 'display_circle_output/display_circle_compare.pdf'
